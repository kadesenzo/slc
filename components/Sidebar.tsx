...
const Sidebar: React.FC<SidebarProps> = ({ role, isOpen, onClose }) => {
  ...
  return (
    <>
      ...
      {isOpen && (
        <div 
          className="fixed inset-0 bg-black/80 backdrop-blur-sm z-[140] transition-opacity duration-300"
          onClick={onClose}
        />
      )}

      <aside className={`
        fixed inset-y-0 left-0 z-[150] w-72 bg-[#0c0c0e] border-r border-zinc-800/50 transition-all duration-500 transform ease-in-out shadow-2xl
      ...
      `}>
        <div className="p-8 flex items-center justify-between">
          ...
          <button 
            onClick={onClose}
            className="p-2 bg-zinc-900 rounded-lg text-zinc-500 hover:text-white transition-colors border border-zinc-800"
          >
            <X size={18} />
          </button>
        </div>
